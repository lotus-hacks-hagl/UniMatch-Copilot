...
| **`[PUBLIC_BASE_URL]/internal/jobs/done`** | **Nhận POST** | Webhook để **AI báo kết quả về cho Backend**. Tham số `PUBLIC_BASE_URL` trong `.env` phải là đường dẫn mà AI Service có thể gọi tới được (VD: ngrok URL hoặc localhost chung network). |
