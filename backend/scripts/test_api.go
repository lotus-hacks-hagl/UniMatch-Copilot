...
func main() {
	...
	resp, err := http.Post("http://localhost:8894/api/v1/auth/login", "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
	...
	if result.Data.Token != "" {
		...
		// Test cases list
		req, _ := http.NewRequest("GET", "http://localhost:8894/api/v1/cases?status=all&page=1&limit=10", nil)
