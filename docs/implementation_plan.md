# UniMatch Copilot — Implementation Plan v2

> Hackathon 24h · 3 người: **Person A (FE)**, **Person B (BE)**, **Person C (AI Service)**
> Stack: Vue3 + Tailwind · Go (Gin) + PostgreSQL + Redis · Python (FastAPI) + Claude Agent SDK + TinyFish MCP + Neo4j
...
[Vue3 FE :5173]
      │ REST poll
      ▼
[Go BE :8894]  ──── fire & forget ──▶  [Python AI Service :8895]
      │                                        │  asyncio.Queue
      │                                        │  ┌───────────────────────────────┐
  [PG BE]                                      │  │  crawl_worker                 │
  ...
  activity_log                                 │  │              fetch_page        │
                                           ...
                                           University nodes
                                           Program / Req / Scholarship
                                           agent-detected entities
      ▲
...
// Agent sẽ research, fill những gì tìm được, giữ null những gì không tìm thấy.
{
  ...
  "callback_url": "http://localhost:8894/internal/jobs/done",
  "metadata": {
...
```go
type Config struct {
    Port          string // default "8894"
    DatabaseURL   string // postgres://...
    AIServiceURL  string // http://localhost:8895
    PublicBaseURL string // http://localhost:8894 (dùng cho callback URL)
}
```
...
> **TinyFish MCP** không cần install package riêng. Được inject vào Claude Agent SDK qua `mcp_servers` parameter khi tạo agent — chỉ cần API key và server URL từ hackathon organizer.
...
graph.py             # Neo4j driver + CRUD operations
job_db.py            # SQLAlchemy — chỉ lưu job state
workers/
  crawl_worker.py    # Claude Agent SDK + TinyFish MCP → Neo4j write
  analyze_worker.py  # Claude + Neo4j query → recommendations
...
TINYFISH_MCP_URL=...           # URL MCP server từ hackathon (e.g. https://mcp.tinyfish.io/sse)
TINYFISH_API_KEY=...           # key để authenticate với MCP server
...
### C-1: Neo4j Knowledge Graph Schema (30m)

**File:** `graph.py`

**Node labels** (predefined — agent có thể tạo thêm, nhưng các label này luôn phải tồn tại):

```
(:University)         — node gốc, key = BE's university_id
(:Program)            — ngành học (CS, Engineering...)
(:AdmissionReq)       — yêu cầu tuyển sinh (IELTS, GPA, SAT...)
(:Scholarship)        — học bổng (tên, amount, điều kiện)
(:City)               — thành phố (agent tự detect từ trang web)
(:Deadline)           — deadline node (intake period + date)
```

**Relationship types** (predefined + agent có thể tạo thêm):
...
(Program)-[:REQUIRES]->(AdmissionReq)        # program-specific requirements
```

**`graph.py` — Neo4j driver setup và core operations:**
...
async def upsert_university_node(university_id: str, props: dict):
    """Upsert University node với BE's ID làm key."""
    async with driver.session() as s:
        await s.run("""
            MERGE (u:University {be_id: $id})
            SET u += $props, u.updated_at = datetime()
        """, id=university_id, props=props)

async def get_university_flat(university_id: str) -> dict:
    """
    Query graph và flatten về dict với các fixed-schema fields.
    Dùng cho analyze_worker để lấy KB data.
    """
    async with driver.session() as s:
        result = await s.run("""
...
async def delete_university_and_orphans(university_id: str):
    """
    Xóa University node và tất cả nodes chỉ connected với university này.
    Nodes connected với nhiều universities (e.g. City) thì giữ lại.
    """
    async with driver.session() as s:
        # Xóa các nodes chỉ có 1 university liên kết
        await s.run("""
            MATCH (u:University {be_id: $id})-[r]->(n)
            WHERE NOT (n)<--(:University {be_id: $id})
              AND size([(n)<--(:University) | n]) <= 1
            DETACH DELETE n
        """, id=university_id)
        # Xóa university node
        await s.run("""
...
async def get_all_universities_flat(filters: dict = None) -> list[dict]:
    """Lấy tất cả universities dạng flat dict để analyze_worker hard filter."""
    query = """
        MATCH (u:University)
        OPTIONAL MATCH (u)-[:REQUIRES]->(r:AdmissionReq)
        OPTIONAL MATCH (u)-[:HAS_PROGRAM]->(p:Program)
...
def _flatten_record(record: dict) -> dict:
    """Convert Neo4j record sang flat dict khớp với fixed-schema fields."""
    u = dict(record["u"])
    reqs = [dict(r) for r in record.get("reqs", [])]
    scholarships = record.get("scholarships", [])

    # Extract IELTS và GPA từ AdmissionReq nodes
    ielts_req = next((r for r in reqs if r.get("type") == "IELTS"), None)
    gpa_req = next((r for r in reqs if r.get("type") == "GPA"), None)
    ...
    return {
        ...
        "ielts_min": ielts_req.get("min_score") if ielts_req else None,
        "sat_required": u.get("sat_required"),
        "gpa_expectation_normalized": gpa_req.get("min_score") if gpa_req else None,
...
**Neo4j constraints** (chạy 1 lần khi startup):
...
### C-4: Crawl Worker — Claude Agent SDK + TinyFish MCP (90m)

**File:** `workers/crawl_worker.py`

Đây là core của AI Service. Agent nhận university metadata (có null fields), tự quyết dùng TinyFish tools để research, tự detect entities, ghi vào Neo4j, rồi callback về BE với fixed-schema đã fill.

**System prompt** (`prompts/crawl_system.txt`):

```
You are a university research agent. Your task is to research a given university
and extract structured admission information.

You have access to web search and page fetch tools via TinyFish.
Use them autonomously — search for admission requirements, fetch official pages,
and verify data until you are confident in the results.

You MUST research and populate the following fields if findable:
- ielts_min (float, e.g. 6.5)
- sat_required (boolean)
- gpa_expectation_normalized (float, 0.0–4.0 scale)
- tuition_usd_per_year (integer, convert to USD: 1 GBP=1.27, 1 EUR=1.08, 1 AUD=0.65, 1 SGD=0.74)
- scholarship_available (boolean)
- scholarship_notes (string, brief description)
- application_deadline (string, YYYY-MM-DD format, nearest upcoming)
- available_majors (array of strings)
- acceptance_rate (float, 0.0–1.0)
- qs_rank (integer)

Additionally, extract any entities and relationships you find that are useful
for a knowledge graph. Predefined types: Program, AdmissionReq, Scholarship,
City, Deadline. You may define new entity types if meaningful.

When done, respond with a JSON object with two keys:
1. "fixed_fields": the structured fields above (null if not found)
2. "graph_entities": array of entity+relationship objects to write to the knowledge graph

Format for graph_entities:
[
  {
    "label": "Program",
    "properties": {"name": "Computer Science", "degree": "MSc"},
    "relationship": "HAS_PROGRAM"
  },
  {
    "label": "AdmissionReq",
    "properties": {"type": "IELTS", "min_score": 7.0, "notes": "All bands min 6.0"},
    "relationship": "REQUIRES"
  },
  {
    "label": "Scholarship",
    "properties": {"name": "Holland Scholarship", "amount_eur": 5000, "notes": "Merit-based"},
    "relationship": "OFFERS"
  },
  {
    "label": "City",
    "properties": {"name": "Delft", "country": "Netherlands"},
    "relationship": "LOCATED_IN"
  },
  {
    "label": "Deadline",
    "properties": {"intake": "Fall 2026", "date": "2026-04-15"},
    "relationship": "HAS_DEADLINE"
  }
]

Be thorough. Use multiple tool calls if needed. Only stop when you have
searched all relevant pages or exhausted available information.
```
...
import anthropic
import json
from graph import upsert_university_node, driver as neo4j_driver
...
async def process_crawl_job(job: dict):
    ...
    user_message = f"""
Research this university and fill in the missing information.
...
Fields to find: {null_fields}

Start by searching for "{metadata['name']} admission requirements {metadata.get('target_year', '2026')}".
"""

    # Run Claude Agent with TinyFish MCP tools
    # Agent loops autonomously until it calls stop_reason = "end_turn"
    ...
    while True:
        response = claude.messages.create(
            ...
            mcp_servers=[
                {
                    "type": "url",
                    "url": config.TINYFISH_MCP_URL,
                    "name": "tinyfish",
                    "authorization_token": config.TINYFISH_API_KEY,
                }
            ],
        )
        ...
        # If tool_use: MCP handles tool execution automatically in SDK
        # We just need to loop back — SDK appends tool results internally
        # But for explicit tool_use blocks, add tool_result to messages
        tool_results = []
        for block in response.content:
            if block.type == "tool_use":
                # MCP tool results come back in next response automatically
                # Track source URLs from tool inputs
                if hasattr(block, "input") and "url" in block.input:
                    source_urls.append(block.input["url"])

        if response.stop_reason == "tool_use" and not tool_results:
            # MCP tools: just continue the loop, SDK handles result injection
            continue
    ...
    # Agent may wrap in ```json ... ``` — strip it
    ...
    graph_entities = agent_output.get("graph_entities", [])

    # --- Write to Neo4j ---

    # 1. Upsert University node with fixed fields
    university_props = {
        "name": metadata["name"],
        "country": metadata["country"],
        **{k: v for k, v in fixed_fields.items()
           if k not in ("available_majors", "scholarship_notes") and v is not None}
    }
    await upsert_university_node(university_id, university_props)

    # 2. Write agent-detected entities and relationships
    async with neo4j_driver.session() as s:
        for entity in graph_entities:
            ...
            # Determine merge key for each entity type
            merge_key = _get_merge_key(label, props)

            cypher = f"""
                MATCH (u:University {{be_id: $uid}})
                MERGE (n:{label} {{{merge_key}: $key_val}})
                SET n += $props
                MERGE (u)-[:{rel}]->(n)
            """
            await s.run(cypher,
                        uid=university_id,
                        key_val=props.get(merge_key),
                        props=props)

    # --- Detect changes vs previously known values ---
    changes = []
    for field, new_val in fixed_fields.items():
    ...
    # --- Callback to BE ---
    result = {
        **metadata,                          # start with original (preserves nulls)
        **{k: v for k, v in fixed_fields.items() if v is not None},  # fill found fields
...
def _get_merge_key(label: str, props: dict) -> str:
    """Return the property to use as merge key for each entity label."""
    keys = {
        "Program": "name",
        "AdmissionReq": "type",
        "Scholarship": "name",
    ...
    # Agent-defined labels: use 'name' if exists, else first key
    return keys.get(label, "name" if "name" in props else list(props.keys())[0])
...
Analyze worker **không dùng Agent SDK** — gọi Claude 1 lần với data từ Neo4j graph. Graph đã được crawl worker xây dựng, nên data luôn fresh.

**Hard filter (pure Python — trước khi gọi Claude):**

```python
from graph import get_all_universities_flat

async def hard_filter(input_data: dict) -> list[dict]:
    all_unis = await get_all_universities_flat()
    filtered = []
    for u in all_unis:
        # Country
        if input_data["preferred_countries"]:
            if u["country"] not in input_data["preferred_countries"]:
                continue
        # Budget
        if not input_data["scholarship_required"] and u["tuition_usd_per_year"]:
            if u["tuition_usd_per_year"] > input_data["budget_usd_per_year"] * 1.4:
                continue
        # Major (loose match)
        if input_data["intended_major"] and u["available_majors"]:
            m = input_data["intended_major"].lower()
            if not any(m[:4] in major.lower() for major in u["available_majors"]):
                continue
        # IELTS hard cutoff
        if input_data.get("ielts_overall") and u["ielts_min"]:
...
**Claude matching prompt + call** — same as v1, unchanged. See `prompts/analyze.txt`.
...
async def process_analyze_job(job: dict):
    ...
    filtered = await hard_filter(input_data)
    if not filtered:
...
import asyncio
from graph import init_graph_constraints
...
@app.get("/health")
async def health():
    from graph import neo4j_driver
    try:
        async with neo4j_driver.session() as s:
...
# 2. Start AI Service trước (vì BE cần gọi nó ngay)
cd ai-service && python main.py
# Verify: curl http://localhost:8895/health
# Expected: { "status": "ok", "graph_university_count": 0 }

# 3. Start BE (tự run migrations + seed)
cd be && go run cmd/server/main.go
# Verify: curl http://localhost:8894/api/v1/universities
# Expected: 5 universities, auto-trigger crawl cho tất cả
# Verify graph sau ~60s: curl http://localhost:8895/health
...
5. `curl http://localhost:8895/health` → `graph_university_count` giảm 1
...
| Neo4j down | crawl/analyze worker fail → callback "failed" |
...
### FE `.env.local`
```
VITE_API_BASE_URL=http://localhost:8894/api/v1
```
...
JOB_DATABASE_URL=postgres://postgres:password@localhost:5433/unimatch_jobs
ANTHROPIC_API_KEY=sk-ant-...
TINYFISH_MCP_URL=https://mcp.tinyfish.io/sse
TINYFISH_API_KEY=...
NEO4J_URI=bolt://localhost:7687
NEO4J_USER=neo4j
NEO4J_PASSWORD=password
...
| C-1 | Neo4j schema + graph.py | C | 30m | P0 |
...
| C-4 | Crawl worker: Agent SDK + TinyFish MCP → Neo4j | C | 90m | P0 |
| C-5 | Analyze worker: hard filter + Claude | C | 60m | P0 |
...
- **Neo4j phải chạy trước AI Service.** `init_graph_constraints()` chạy khi startup, nếu Neo4j chưa ready sẽ fail.
...
- **TinyFish MCP URL**: Confirm với hackathon organizer xem endpoint là SSE (`/sse`) hay HTTP streaming. Claude Agent SDK hỗ trợ cả hai qua `"type": "url"`.
- **Agent loop timeout**: Crawl agent có thể mất 30–60s/university (nhiều tool calls). `B-3 AIClient` nên set timeout cho crawl job submit lên 15s (chỉ chờ `accepted`), không phải chờ kết quả.

 
 - - - 
 
 # #   S p r i n t   2 :   A u t h e n t i c a t i o n ,   R B A C   &   T e a c h e r   M a n a g e m e n t   ( N e w   P l a n ) 
 
 # # #   C - 1 :   B a c k e n d   A u t h   &   A d m i n   ( P e r s o n   B ) 
 -   * * S c h e m a : * *   T h � m   b �n g   \ u s e r s \   ( e m a i l ,   m �t   k h �u   b m ,   r o l e ,   i s _ v e r i f i e d ) .   U p d a t e   \ c a s e s \   � n h   x �  1 - n   v �i   \ u s e r s \ . 
 -   * * A P I   ( A u t h e n t i c a t i o n ) : * *   n g   n h �p ,   n g   k �   ( t r �  J W T   q u a   \ h t t p O n l y \   c o o k i e   h o �c   A u t h o r i z a t i o n   H e a d e r ) . 
 -   * * A P I   ( A d m i n ) : * *   L i �t   k �   T e a c h e r s ,   A p p r o v e / R e j e c t   t � i   k h o �n   n g   k � . 
 -   * * M i d d l e w a r e : * *   C o d e   V a l i d a t o r   c h o   q u y �n   \ A d m i n \   v �   \ T e a c h e r \ . 
 
 # # #   C - 2 :   F r o n t e n d   A u t h   F l o w   ( P e r s o n   A ) 
 -   * * M � n   h � n h   L o g i n / R e g i s t e r : * *   D e s i g n   l a y o u t   A u t h   2   c �t .   F o r m   v a l i d a t i o n . 
 -   * * P i n i a   S t o r e : * *   S e t u p   \  u t h S t o r e \   �  l �u   t h � n g   t i n   \ 
 o l e \   v �   s t a t u s   \  e r i f i e d \ . 
 
 # # #   C - 3 :   A d m i n   M a n a g e m e n t   D a s h b o a r d   ( P e r s o n   A ) 
 -   * * G i a o   d i �n   T e a c h e r   L i s t : * *   F i l t e r   c � c   T e a c h e r   c h �  d u y �t   - >   A c t i o n   A p p r o v e   ( D u y �t ) / S u s p e n d   ( n g   x u �t   n g a y ) . 
 -   * * D a s h b o a r d   P h � n   l u �n g : * *   N �u   r o l e   =   A d m i n   - >   x e m   t �t   c �  c a s e ;   N �u   r o l e   =   T e a c h e r   - >   c h �  x e m   \ / c a s e s ? a s s i g n e d _ t o = m e \ . 
 
 
 