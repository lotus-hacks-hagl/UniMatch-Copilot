...
<template>
  <div class="px-7 py-6 max-w-[1400px] mx-auto">
    <div v-if="loading" class="text-text-muted mt-10 text-center">Loading case details...</div>
    ...
    <div v-else-if="caseData" class="space-y-6">
      ...
      <div class="flex items-center gap-3 text-[13px]">
        ...
        <span class="font-medium text-text">{{ caseData.student?.full_name || 'Student' }}</span>
        ...
        <div class="w-full xl:w-[320px] shrink-0 space-y-4">
          ...
          <div class="bg-surface rounded-xl p-5 border border-primary/20 shadow-sm relative overflow-hidden">
            ...
            <div class="flex items-start justify-between mb-4">
              <div class="text-[13px] font-medium text-text mt-1">AI Match Verdict</div>
            ...
            <p class="text-[13px] text-text-muted leading-relaxed">
              {{ caseData.status === 'human_review' ? caseData.escalation_reason || 'Escalated to human review due to unhandled edge cases.' : 'Auto-approved profile based on historical admits.' }}
            </p>
          </div>
          ...
          <div class="bg-surface rounded-xl p-5 border border-black/5 shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-y-5 gap-x-4">
              <div>
                <div class="text-[11px] text-text-muted mb-0.5 uppercase tracking-wider">GPA (Norm)</div>
                <div class="text-[14px] font-bold text-text">{{ caseData.student?.gpa_normalized || 'N/A' }}</div>
              </div>
              <div>
                <div class="text-[11px] text-text-muted mb-0.5 uppercase tracking-wider">IELTS</div>
                <div class="text-[14px] font-bold text-text">{{ caseData.student?.ielts_overall || 'N/A' }}</div>
              </div>
              <div>
                <div class="text-[11px] text-text-muted mb-0.5 uppercase tracking-wider">Budget</div>
                <div class="text-[14px] font-bold text-text">{{ formatBudget(caseData.student?.budget_usd_per_year) }}</div>
              </div>
              <div>
                <div class="text-[11px] text-text-muted mb-0.5 uppercase tracking-wider">Countries</div>
                <div class="text-[14px] font-bold text-text truncate max-w-[120px]" :title="(caseData.student?.preferred_countries||[]).join(', ')">
                  {{ (caseData.student?.preferred_countries||[]).join(', ') || 'Any' }}
                </div>
              </div>
            </div>
            ...
            <div v-if="caseData.profile_summary" class="mt-6 pt-5 border-t border-black/5 space-y-4">
               <div>
                 <div class="text-[11px] text-text-muted mb-2 uppercase tracking-wider">Strengths</div>
               ...
               <div>
                 <div class="text-[11px] text-text-muted mb-2 uppercase tracking-wider">Risks</div>
                 <div class="flex flex-wrap gap-1.5">
          ...
          <div class="p-6 flex-1 bg-bg/20">
            ...
            <div v-if="activeTab === 'Profile'">
              <div class="bg-surface rounded-xl p-6 border border-black/10">
                <h3 class="text-base font-bold text-text mb-4">Full Student Background</h3>
                <div class="space-y-6 text-[13px]">
                  <div>
                    <div class="font-medium text-text mb-1 relative pb-2"><span class="absolute bottom-0 left-0 w-8 h-0.5 bg-primary/30"></span>Extracurriculars</div>
                    <p class="text-text-muted leading-relaxed whitespace-pre-wrap">{{ caseData.student?.extracurriculars || 'None recorded' }}</p>
                  </div>
                  <div>
                    <div class="font-medium text-text mb-1 relative pb-2"><span class="absolute bottom-0 left-0 w-8 h-0.5 bg-primary/30"></span>Achievements</div>
                    <p class="text-text-muted leading-relaxed whitespace-pre-wrap">{{ caseData.student?.achievements || 'None recorded' }}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div v-if="activeTab === 'AI Analysis'">
              <div v-if="['pending', 'processing'].includes(caseData.status)" class="flex flex-col items-center justify-center h-[300px]">
                ...
                <div class="text-[13px] font-medium text-primary">AI is analyzing profile and sourcing universities...</div>
              </div>
              <div v-else-if="!caseData.recommendations || caseData.recommendations.length === 0" class="text-center mt-10 text-text-muted text-[13px]">
                No recommendations generated for this case.
              </div>
                ...
                <div v-for="rec in [...caseData.recommendations].sort((a,b)=>a.rank_order - b.rank_order)" :key="rec.id" 
                  ...
                  <div class="flex items-center gap-6 text-[12px] font-medium text-text-muted mt-4">
                    <div class="flex items-center gap-1.5 bg-bg px-2.5 py-1 rounded-md border border-black/5">
                      ...
                      Likelihood: {{ rec.admission_likelihood_score }}%
                    </div>
                    <div class="flex items-center gap-1.5 bg-bg px-2.5 py-1 rounded-md border border-black/5">
            ...
            <div v-else class="flex flex-col items-center justify-center p-12 opacity-50">
               ...
               <div class="text-[13px] font-medium text-text-muted">Section Under Construction</div>
            </div>
